...
$(document).ready(function() {
    ...
    $(".prs-toc-link").on("click", function(event) {
        event.preventDefault();
        const target = $(this.hash);

        if (target.length) {
            const scrollTop =
                document.body.scrollTop + target.offset().top - 125;
    ...
    var addClassOnScroll = function() {
        ...
        $("section[id]").each(function(index, elem) {
            var offsetTop = $(elem).offset().top - 160;
            ...
            if (
                ...
                $(
                    "nav ul li a[href='https://www.tesvan.com/#" + elemId + "']"
                ).addClass("current");
            }
        });
    };
